    ...
    public class RecipeValidator : AbstractValidator<RequestRecipeJson>
    {
        public RecipeValidator()
        {
            ...
            RuleFor(recipe => recipe.Instructions.Count).GreaterThan(0)
                .WithMessage(ResourceMessagesException.AT_LIST_ONE_INSTRUCTION);
            ...
            RuleForEach(recipe => recipe.Instructions).ChildRules(instructionRules =>
            {
                instructionRules.RuleFor(instruction => instruction.Step).GreaterThan(0)
                    .WithMessage(ResourceMessagesException.NON_NEGATIVE_OR_ZERO_INSTRUCTION_STEP);
                instructionRules.RuleFor(instruction => instruction.Text).NotEmpty()
